...
describe("P2.13 — buildPartnerPrompt rendering", () => {
  it("renders coreIdentityBlock verbatim BEFORE 'You are Luca'", () => {
    ...
    expect(coreIdx).toBeLessThan(lucaIdx);
...
describe("P2.13 — callsite integration", () => {
  ...
  it("coreIdentityBlock is passed as last argument to buildPartnerPrompt in partner-chat branch", () => {
    // The buildPartnerPrompt call must pass the block as its final arg
    const callsite = extractBlock(source, "const systemPrompt = isPartnerChat", "      // Build conversation history");
    expect(callsite).toMatch(/buildPartnerPrompt\([^)]*coreIdentityBlock\s*\)/);
  });
