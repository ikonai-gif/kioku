...
describe("remember tool — handler invariants", () => {
  ...
  it("INSERT uses the callsite userId and agentId bindings", () => {
    ...
    // provenance/verified are LITERALS ('luca_inferred', false) hard-coded in the SQL —
    // NOT placeholders — so Luca cannot override them from toolInput.
    expect(handler).toMatch(
      /VALUES\s*\(\s*\$1\s*,\s*\$2\s*,\s*\$3\s*,\s*\$4\s*,\s*\$5\s*,\s*\$6\s*,\s*\$7\s*,\s*\$8\s*,\s*'luca_inferred'\s*,\s*false\s*,\s*\$9\s*,\s*\$10\s*,\s*\$11\s*,\s*\$12\s*,\s*\$13\s*,\s*\$14\s*\)/
    );
