...
describe("remember tool — handler invariants", () => {
  ...
  it("INSERT uses the callsite userId and agentId bindings", () => {
    // Sprint 1 v2: INSERT had 10 placeholders (added provenance, verified, confidence).
    // LUCA-048 (PR #151): added $11=expiresAt for TTL policy.
    // [BRO2-280] (CCP Phase 1.0): added $12=temporalStance for ccp_y axis.
    // [BRO2-325] (bi-temporal 2.1a): added $13=factKey, $14=validFrom.
    // $1=userId, $2=agentId, ... $9=confidence, $10=now, $11=expiresAt, $12=temporalStance,
    // $13=factKey, $14=validFrom.
    // [P2 tool_observed]: provenance is now a positional param ($15 = computedProvenance),
    // system-computed from whitelisted prior tools — NOT from toolInput. verified stays
    // a hard-coded literal false. Luca still cannot override either from toolInput.
    expect(handler).toMatch(
      /VALUES\s*\(\s*\$1\s*,\s*\$2\s*,\s*\$3\s*,\s*\$4\s*,\s*\$5\s*,\s*\$6\s*,\s*\$7\s*,\s*\$8\s*,\s*\$15\s*,\s*false\s*,\s*\$9\s*,\s*\$10\s*,\s*\$11\s*,\s*\$12\s*,\s*\$13\s*,\s*\$14\s*\)/
    );
    // $15 = computedProvenance (tool_observed | luca_inferred), derived not from toolInput
    expect(handler).toMatch(/computedProvenance/);
