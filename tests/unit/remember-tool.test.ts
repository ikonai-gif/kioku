...
describe("remember tool — handler invariants", () => {
  ...
  it("INSERT uses the callsite userId and agentId bindings", () => {
    ...
    // LUCA-048 (PR #151): added $11=expiresAt for TTL policy. Total 11 placeholders.
    // $1=userId, $2=agentId, ... $9=confidence, $10=now, $11=expiresAt.
    ...
    expect(handler).toMatch(
      /VALUES\s*\(\s*\$1\s*,\s*\$2\s*,\s*\$3\s*,\s*\$4\s*,\s*\$5\s*,\s*\$6\s*,\s*\$7\s*,\s*\$8\s*,\s*'luca_inferred'\s*,\s*false\s*,\s*\$9\s*,\s*\$10\s*,\s*\$11\s*\)/
    );
