...
// ── Chat Message Bubble ──────────────────────────────────────────
function ChatBubble({ message, isUser, emotion }: { message: any; isUser: boolean; emotion: string }) {
  const glowColor = getGlowColor(emotion);
  return (
    <motion.div
...
export default function PartnerChat() {
  ...
  const send = useCallback(() => {
    if (!input.trim() || !partnerRoomId) return;
    sendMutation.mutate({
      agentId: null,
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain py-3 space-y-1">
        {msgsLoading ? (
          ...
          messages.map((msg: any) => (
            <ChatBubble key={msg.id} message={msg} isUser={isUser(msg)} emotion={emotion} />
          ))
        )}
      ...
      >
        <div className="flex items-end gap-2">
          ...
          <Button
            ...
            style={{
              background: input.trim() ? "#C9A340" : "rgba(201,163,64,0.2)",
              color: input.trim() ? "#0a0f1e" : "rgba(201,163,64,0.5)",
            }}
            onClick={send}
            disabled={!input.trim() || !partnerRoomId || sendMutation.isPending}
          >
