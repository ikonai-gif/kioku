...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  // ── Honesty Layer: Luca capabilities truth table ─────────────
  // Returns the real, env-effective scope of tools Luca can call on prod,
  // plus the last observed tool_activity for room 151 as a "recently fired"
  // signal. Auth: master-key (userId=10). Read-only. Never lists tools that
  // are schema-advertised but not actually dispatchable.
  app.get("/api/admin/luca-capabilities", asyncHandler(async (req, res) => {
    const userId = await getUser(req);
    if (!userId) return res.status(401).json({ error: "Unauthorized" });
